...
class UserList {
	 ...
	 */
	var $user_template = '<div class="{class} clearfix">{user}</div>';
		...
		* @see http://wordpress.org/support/topic/309878 */
		...
		if ($biography) {
			...
			if ($this->show_name || $this->show_bbpress_post_count || $this->show_postcount) {
				...
				$user_roles = get_user_meta($user->user_id, 'ce_capabilities', true );
				...
				foreach((array)$user_roles as $role){
					...
					if ($role != 0){
						...
						echo '<strong>Current User Role</strong>:' . $role;
					}
				}				

				$saved = get_user_meta($user->user_id, '_author_cat', true );

	            foreach((array)$saved as $cq){
	            	$cq = intval($cq);
	            	if ($cq != 0){

		            	$category = get_the_category(); 
		            	$category_link = get_category_link( $cq );
		            	$category_name = get_the_category_by_ID( $cq );
		            	$html .= '<h3><a href="'.esc_url( $category_link ).'" title="Category Name">'.$category_name.'</a></h3>';
	            	}
	            }
